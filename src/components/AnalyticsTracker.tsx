    ...
    const sendWithIP = async () => {
      ...
      try {
        ...
        if (geo.data && !geo.data.error && geo.data.city) {
          geoLocation = {
            ip: geo.data.ip, city: geo.data.city, country: geo.data.country_name,
            countryCode: geo.data.country_code, region: geo.data.region,
            lat: geo.data.latitude, lon: geo.data.longitude,
            isp: geo.data.org || "", source: "ip"
          };
        }
      } catch { /* try next */ }
      ...
      if (!geoLocation.city) {
        try {
          ...
          if (geo2.data?.city) {
            ...
            geoLocation = {
              ip: geo2.data.ip, city: geo2.data.city, country: geo2.data.country,
              countryCode: geo2.data.country, region: geo2.data.region,
              lat, lon, isp: geo2.data.org || "", source: "ip"
            };
          }
        } catch { /* geo failed */ }
      }
